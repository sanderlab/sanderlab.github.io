  ...
  function ($mdThemingProvider, $urlRouterProvider, $stateProvider) {
    ...
    var backController = function ($scope, $element, $state, $stateParams) {
      ...
      if ($scope.params.personId) {
        //helper function to add person if person page.
        $scope.person = _.find($scope.sitedata.people, function (person) {
          if ($scope.params.personId == person.key) {
            return true;
          }
          return false;
        });
...
app.run([
  ...
  function ($rootScope, $log, $transitions, $state) {
    var orderedstatenames = _.reduce(
      $state.get(),
      function (memo, state) {
        if (!state.isTab) {
    ...
    var goBackStates = _.reduce(
      orderedstatenames,
      function (memo, state, idx) {
        if (idx > 0) {
    ...
    $transitions.onStart({}, function (transition) {
      ...
      if (
        _.isArray(backStates) &&
        backStates.indexOf(transition.to().name) >= 0
      ) {
...
app.controller("MainCtrl", [
  ...
  function (
    ...
    $scope.reverseEmail = function (addr) {
      ...
      return (
        nameDomainArr[0].split("").reverse().join("") + "@" + nameDomainArr[1]
      );
    };

    //set the pages configured for this site into the scope.
    $scope.pages = _.reduce(
      $state.get(),
      function (memo, state) {
        if (state.isTab) {
          memo.push({
    ...
    $transitions.onFinish({}, function (transition) {
      if (transition.to().name == "personpage") {
        //special case for person page
        //$scope.selectedTabIndex = 1;
        $scope.selectedTabIndex = _.findIndex($scope.pages, function (p) {
          if (p.state == "people") {
            return true;
          } //make sure people tab is selected
        });
        return;
      }
